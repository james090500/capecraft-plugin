...
public class RandomTP {
	...
	public void teleportPlayer(Player player) {
		// Make sure the player is in the survival world
		if(player.getWorld().getName() == "survival") {
			...
			if(TimeUnit.MILLISECONDS.toSeconds(timePast) >= CooldownManager.DEFAULT_COOLDOWN) {
				...
				player.teleport(new Location(player.getWorld(), player.getLocation().getX() + RAND_X, SAFE_Y, player.getLocation().getZ() + RAND_Z));
